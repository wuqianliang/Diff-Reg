...
class Trainer(object):
    ...
    def inference_one_batch(self, inputs, phase):
        ...
        if (phase == 'train'):
            ...
            self.model.eval()
            with torch.no_grad():
                data = self.model(inputs, timers=self.timers, eval_flag=True)  # [N1, C1], [N2, C2]
                loss_info = self.loss(data)
    ...
    def inference_one_epoch(self, epoch, phase):
        ...
        num_iter = int(len(self.loader[phase]))#num_iter = int(len(self.loader[phase].dataset) // self.loader[phase].batch_size) # drop last incomplete batch
        ...
        for c_iter in tqdm(range(num_iter)):  # loop through this epoch
            ...
            if phase == 'train' :
                if self.local_rank <= 0 and (c_iter + 1) % self.verbose_freq == 0 and self.verbose  :
