    ...
    def test_thr(self, conf_threshold=None):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                IR += bs*ir
                FMR += (ir>0.05).float()
                # print(idx+1, success1/(idx+1), IR.item()/(idx+1), FMR.item()/(idx+1))
    ...
    def test_thr(self, conf_threshold=None):
        ...
        with torch.no_grad():
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                # match_pred, _, _ = CM.get_match(data['conf_matrix_pred'], thr=conf_threshold, mutual=True)
                match_pred = data['match_pred']
                # match_pred, _, _ = CM.get_match(data['conf_matrix_pred'], thr=0.55, mutual=True)
                ...
                # print(idx+1, NR_FMR.item()/(idx+1), IR.item()/(idx+1))
