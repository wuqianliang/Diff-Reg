...
class KPFCN(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.transformer_architecture = ['self','cross','self','cross','self','cross']
        # self.global_transformer = GeometricTransformer(2048, 2048, 256, 4, self.transformer_architecture, sigma_d=0.2, sigma_a=15, angle_k=3, )
    ...
    def forward(self, batch, phase = 'encode'):
        ...
        if phase == 'coarse' :
            ...
            # lens_c = batch['stack_lengths'][-1]
            # points_c = batch['points'][-1]
            # src_pts_c = points_c[:lens_c[0]].unsqueeze(0)
            # tgt_pts_c = points_c[lens_c[0]:].unsqueeze(0)
            # src_fea_c = x[:lens_c[0]].unsqueeze(0)
            # tgt_fea_c = x[lens_c[0]:].unsqueeze(0)
                           
            # # Here insert geometric feature
            # s_g_x4, t_g_x4 = self.global_transformer(src_pts_c, tgt_pts_c, src_fea_c, tgt_fea_c)
            # src_fea_c = src_fea_c + F.relu(s_g_x4)
            # tgt_fea_c = tgt_fea_c + F.relu(t_g_x4)

            # x = torch.cat([src_fea_c, tgt_fea_c], dim=1).squeeze(0)
