...
class CrossModalFusionModule(nn.Module):
    def __init__(
        ...
        self.img_in_proj_dino = nn.Linear(img_input_dim*2, hidden_dim)
        self.img_in_proj_all = nn.Linear(img_input_dim, hidden_dim) 
    ...
    def forward(
        ...
        """
        img_tokens = torch.relu(torch.cat([self.img_in_proj(img_feats), self.img_in_proj_dino(img_feats_dino)], dim=-1)) # (B, HxW, Ci) -> (B, HxW, C)
        img_tokens = self.img_in_proj_all(img_tokens)
