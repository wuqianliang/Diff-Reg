...
class SuperPointMatchingMutualTopk(nn.Module):
    def __init__(self, num_correspondences, k, threshold=None, sample_num=2048, mutual=True, eps=1e-8):
        super().__init__()
    ...
    def forward(self, src_feats, tgt_feats, src_masks=None, tgt_masks=None, init_matching_matrix= None):
        ...
        if self.threshold is not None:
            ...
            masks = torch.le(corr_scores, self.threshold).cpu().numpy()
            print('epnp',masks.sum().item())
            if masks.sum().item() < num_correspondences:
                ...
                src_corr_indices = src_corr_indices[topk_indices.cpu()]
                tgt_corr_indices = tgt_corr_indices[topk_indices.cpu()]
            else:
...
class SuperPointMatchingDenoising(nn.Module):
